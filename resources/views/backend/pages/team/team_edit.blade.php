      ...
      <form action="{{ route('updateteam') }}" method="post" enctype="multipart/form-data">
        ...
        <div class="row">
          ...
          <div class="col-md-12 mb-3">
            ...
            <textarea name="position" rows="5" class="ckeditor form-control @error('position') is-invalid @enderror" id="position" placeholder="Enter Member Position">{{ $team->position }}</textarea>
            @error('position')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        </div>
